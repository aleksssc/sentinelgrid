...
$source = Join-Path $PSScriptRoot 'SentinelGridVideo.cpp'; $h264Source = Join-Path $PSScriptRoot 'SentinelGridH264.cpp'; $decoderSource = Join-Path $PSScriptRoot 'SentinelGridH264Decoder.cpp'; $smokeSource = Join-Path $PSScriptRoot 'h264_smoke.cpp'; $resourceSource = Join-Path $PSScriptRoot 'SentinelGridVideo.rc'; $output = Join-Path $OutputDirectory 'SentinelGridVideo.dll'; $object = Join-Path $OutputDirectory 'SentinelGridVideo.obj'; $h264Object = Join-Path $OutputDirectory 'SentinelGridH264.obj'; $decoderObject = Join-Path $OutputDirectory 'SentinelGridH264Decoder.obj'; $resource = Join-Path $OutputDirectory 'SentinelGridVideo.res'; $importLibrary = Join-Path $OutputDirectory 'SentinelGridVideo.lib'; $smoke = Join-Path $OutputDirectory 'SentinelGridH264Smoke.exe'
$command = 'call "{0}" -arch=amd64 -host_arch=amd64 >nul && rc.exe /nologo /dSG_VERSION_MAJOR={11} /dSG_VERSION_MINOR={12} /dSG_VERSION_PATCH={13} /fo "{6}" "{5}" && cl.exe /nologo /std:c++17 /EHsc /O2 /DWIN32_LEAN_AND_MEAN /c /Fo"{2}" "{1}" && cl.exe /nologo /std:c++17 /EHsc /O2 /DWIN32_LEAN_AND_MEAN /c /Fo"{3}" "{7}" && cl.exe /nologo /std:c++17 /EHsc /O2 /DWIN32_LEAN_AND_MEAN /c /Fo"{4}" "{8}" && link.exe /nologo /DLL /OUT:"{9}" /IMPLIB:"{14}" "{2}" "{3}" "{4}" "{6}" d3d11.lib d3dcompiler.lib dxgi.lib mfplat.lib mfuuid.lib ole32.lib oleaut32.lib user32.lib && cl.exe /nologo /std:c++17 /EHsc /O2 /DWIN32_LEAN_AND_MEAN /Fe:"{10}" "{15}" mfplat.lib mfuuid.lib ole32.lib' -f $devCmd,$source,$object,$h264Object,$decoderObject,$resourceSource,$resource,$h264Source,$decoderSource,$output,$smoke,$versionParts[0],$versionParts[1],$versionParts[2],$importLibrary,$smokeSource
cmd.exe /d /s /c $command
if ($LASTEXITCODE -ne 0) { throw "Native x64 DLL build failed (exit $LASTEXITCODE)." }
if (-not (Test-Path -LiteralPath $output) -or -not (Test-Path -LiteralPath $smoke)) { throw 'Native build did not produce the DLL and H.264 smoke executable.' }
...
Write-Host "SentinelGridVideo.dll built: $output"; Write-Host "SentinelGridH264Smoke.exe built: $smoke"; Write-Host "VERSIONINFO $Version"; Write-Host 'BUILD_TARGET x64'
