...
import { authorizeRDPViewerDownload, browserRDPSession, RDPError, rdpError } from "@/lib/remote/rdp";
...
export async function GET(request: NextRequest) {
 try {
  ...
  const organizationId=sessionId
   ? (await browserRDPSession(deviceId,sessionId)).organization_id
   : await authorizeRDPViewerDownload(deviceId);
  const admin=createAdminClient();
  const {data:policy,error:policyError}=await admin.from("organization_agent_update_settings").select("channel").eq("organization_id",organizationId).maybeSingle(); if(policyError)throw new Error("DOWNLOAD_POLICY_FAILED");
