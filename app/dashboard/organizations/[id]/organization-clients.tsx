...
export default function OrganizationClients({ organizationId, clients }: Props) {
  ...
  return <div className="sg-client-directory" aria-busy={!ready}>
    <div className="sg-toolbar sg-client-toolbar">
      <div className="relative min-w-0 flex-1 sm:max-w-xl"><Search size={16} aria-hidden="true" className="absolute left-3 top-1/2 -translate-y-1/2 text-surface-muted" /><input type="search" aria-label="Search clients" placeholder="Search clients..." value={search} onChange={(event) => setSearch(event.target.value)} className="sg-control w-full pl-9 pr-3" /></div>
