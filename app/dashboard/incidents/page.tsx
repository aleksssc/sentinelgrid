...
export default async function IncidentsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const filters = parseOperationsFilters(await searchParams, "incidents");
  const now = Date.now();
  const result = await loadIncidents(organization.id, filters, now);
  return <OperationsView kind="incidents" organizationName={organization.name} filters={filters} result={result} now={now} />;
}
