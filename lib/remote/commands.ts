...
const HIGH_RISK_ACTIONS = new Set<QuickAction>([
  "update_agent",
  "reboot",
  "shutdown",
  "lock",
...
function isQuickAction(value: unknown): value is QuickAction {
  return typeof value === "string" && QUICK_ACTIONS.includes(value as QuickAction);
}

function validatePayload(commandType: QuickAction, payload: unknown): CommandPayload {
  ...
  if ("delay_seconds" in input) {
    if (
      typeof input.delay_seconds !== "number" ||
  ...
  if (
    ...
    ("delay_seconds" in result || "force" in result)
  ) {
    throw new Error("INVALID_PAYLOAD");
  }
...
export async function createQuickAction({
  ...
  if (!isQuickAction(commandType)) throw new Error("INVALID_COMMAND_TYPE");

  await enforceRemoteRateLimit(user.id, "commands");

  const safePayload = validatePayload(commandType, payload);
  ...
  const { data: device } = await supabase
    .from("devices")
    .select("id, hostname, display_name, status, last_seen, client_id, capabilities")
    .eq("id", deviceId)
    .maybeSingle();
  ...
  if (commandType === "update_agent" &&
    (capabilities.agent_update !== true || process.env.SENTINELGRID_AGENT_UPDATES_ENABLED !== "true")) {
    throw new Error("AGENT_UNSUPPORTED");
  }
  ...
  if (existingCommand) {
    return {
      ...
      expiresAt: existingCommand.expires_at,
    };
  }
  ...
  if (HIGH_RISK_ACTIONS.has(commandType)) {
  ...
  const { data: settings } = await supabase
    ...
    .select("remote_access_enabled, terminal_enabled")
    .eq("organization_id", organization.id)
    .maybeSingle();

  if (settings && !settings.remote_access_enabled) throw new Error("REMOTE_ACCESS_DISABLED");
  ...
  await publishRealtimeMessage(agentCommandChannel(device.id), {
    type: "typed_command",
  ...
  await createAuditLog({
    ...
    targetType: "device",
    targetId: device.id,
    targetName: device.display_name || device.hostname,
    metadata: {
