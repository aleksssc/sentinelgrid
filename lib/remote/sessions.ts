...
function isAal2(user: { id: string }, assuranceLevel: string | null | undefined) {
  return Boolean(user.id) && assuranceLevel === "aal2";
}
...
export async function createRemoteSession({
  ...
  if (!isSessionType(sessionType)) throw new Error("INVALID_SESSION_TYPE");
  ...
  if (!isAal2(user, assurance?.currentLevel)) {
    throw new Error("AAL2_REQUIRED");
  }

  const { data: device } = await supabase
    .from("devices")
    .select("id, hostname, display_name, client_id, last_seen, capabilities")
    .eq("id", deviceId)
    .maybeSingle();
  ...
  if (settings && !settings.remote_access_enabled) {
    throw new Error("REMOTE_ACCESS_DISABLED");
  }
  ...
  const { count } = await supabase
    ...
    .eq("organization_id", organization.id)
    .in("status", ["requested", "connecting", "active"]);

  if ((count ?? 0) >= (settings?.max_concurrent_sessions ?? 2)) {
    throw new Error("SESSION_LIMIT_REACHED");
  }

  const maxMinutes = settings?.max_session_minutes ?? 30;
  ...
  await createAuditLog({
    ...
    action: `remote.${sessionType}.requested`,
    targetType: "device",
    targetId: device.id,
