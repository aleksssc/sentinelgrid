        ...
        async (
          ...
          try {
            ...
            if (
              ...
              subscription =
                subscribeRealtimeChannel<RemoteCommand | TypedRemoteCommand | { type: "rdp_available" }>(
                  agentCommandChannel(
                    deviceId,
                  ),
                  async (
                    ...
                    if (command.type !== "command" && command.type !== "typed_command" && command.type !== "rdp_available") {
                      return;
                    }
                    ...
                    try {
                      ...
                      console.log(
                        `[Realtime Agent] Command forwarded to ${hostname}: ${"command_id" in command ? command.command_id : "rdp_available"}`,
                      );
                    } catch (
                      error
                    ) {
