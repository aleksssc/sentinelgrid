...
function fixture(){
 ...
 for(const [key,filename]of Object.entries({agent:"SentinelGridAgent.exe",updater:"SentinelGridUpdater.exe",rdp_client:"SentinelGridRDP.exe",msi:"SentinelGridAgent.msi"}))manifest[key]={filename,version:"0.1.9",sha256:(key==="msi"?"b":"a").repeat(64),size:42};
...
for(const key of ["agent","updater","rdp_client","msi"])test(`publication and delivery reject mixed ${key} version`,()=>{
...
test("both triggers converge before download and old agents never receive an MSI URL",()=>{
 ...
 assert.match(api,/if \(!release \|\| !enabled \|\| !capability \|\| !compatible\)/);assert.ok(api.indexOf("!compatible) {")<api.indexOf("storage.createSignedUrl(artifact.storage_path"));
});
test("MSI preserves enrollment, retains Windows Installer rollback and includes all binaries",()=>{
 ...
 for(const component of ["Agent","Updater","RDP"])assert.match(wix,new RegExp(`ComponentRef Id="SentinelGrid${component}Component"`));
 assert.match(wix,/MajorUpgrade Schedule="afterInstallExecute"/);
 assert.match(wix,/NOT Installed AND NOT WIX_UPGRADE_DETECTED AND NOT SG_EXISTING_CONFIG/);
 assert.ok(!/<(?:File|RemoveFile|RemoveFolder)\b[^>]*(?:agent\.json|CommonAppDataFolder)/.test(wix));
 assert.match(wix,/ExeCommand="-maintenance-rollback" Execute="rollback"/);
});
