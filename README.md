...
Device Actions uses the shared collision-aware, portalled menu with viewport-bounded scrolling. The remote terminal uses a native modal (`ViewportDialog`) sized to the visual viewport, so the iOS keyboard cannot push its close button outside the screen. Its header wraps on phones, input text stays at least 16px to avoid Safari focus zoom, and output scrolls only inside the terminal. Safe-area padding, touch-sized dismissal, focus containment/restoration and reduced motion are preserved. Device headers use tighter mobile spacing; on short landscape screens they scroll with the details rather than covering the content. Add `--device-only` to the browser command above for the device touch suite (all sections, menu collisions, busy-terminal dismissal, focus restoration, long output and simulated visual-viewport resize/pan). These Chromium fixtures do not emulate the native iOS keyboard or replace Safari testing on a physical iPhone.

The mobile navigation keeps its native modal/backdrop stationary and animates only the drawer panel. Its dim overlay intentionally has no backdrop blur; decorative dashboard animations pause while the menu is open to avoid continuously filtering/repainting the background on mobile. Touch regression checks use the real dashboard background and verify repeated open/close cycles, all desktop sidebar preferences, reduced motion and responsive page controls after dismissal.
