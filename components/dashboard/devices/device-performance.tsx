  ...
  return (
    <section className="mt-6 space-y-4" aria-label="Device performance" aria-busy={loading}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <AnimatedSelection value={range} className="sg-segments" role="group" aria-label="Performance period">
        ...
        <button type="button" onClick={() => setRevision((value) => value + 1)} disabled={loading}
          className="sg-button sg-button-secondary sg-button-sm disabled:opacity-50">
      ...
      {!data && !current?.error && <div role="status" className="sg-surface p-12 text-center text-sm text-zinc-400">Loading performance history...</div>}
      {data && !latest && <div role="status" className="sg-surface border-dashed p-10 text-center">
        ...
        <p className="mx-auto mt-2 max-w-md text-xs leading-5 text-surface-muted">No measurements in this period yet. An online Agent sends metrics with its heartbeats, normally every 30 seconds. History starts when collection is enabled; previous values cannot be recovered.</p>
      </div>}
      {data && latest && <>
        ...
        <p className="text-xs leading-5 text-surface-muted">{data.samples.length} recorded samples. Latest measurement per {data.interval < 60_000 ? "30-second" : `${data.interval / 60_000}-minute`} interval, not an average. Gaps indicate missing samples. Disk shows capacity used, not I/O. Refreshes every 30 seconds only while this tab is visible.</p>
      </>}
    </section>
  );
}
...
export function PerformanceChart({ data, metric, label, color, icon: Icon }: {
  ...
  return (
    <div className="sg-surface p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><h3 className="flex items-center gap-2 text-sm font-medium text-zinc-300"><Icon size={15} style={{ color }} />{label}</h3>
          <p className="mt-2 text-2xl font-semibold tracking-tight text-white">{percent(latest)}</p><p className="mt-1 text-[10px] text-surface-muted">Last sample</p></div>
        <p className="text-xs text-surface-muted">Sample avg <span className="text-zinc-300">{percent(average)}</span><span className="mx-2">/</span>Peak <span className="text-zinc-300">{percent(peak)}</span></p>
      </div>
      {values.length ? <svg viewBox="0 0 700 195" className="mt-3 w-full" role="img" aria-label={`${label}: ${percent(latest)} at last sample; sample average ${percent(average)}, peak ${percent(peak)}`}>
        {[0, 50, 100].map((value) => <g key={value}><line x1="36" x2="676" y1={y(value)} y2={y(value)} stroke="#27272a" strokeDasharray="3 5" /><text x="27" y={y(value) + 3} textAnchor="end" fill="#959ca8" fontSize="10">{value}%</text></g>)}
        ...
        {[data.from, (data.from + data.to) / 2, data.to].map((time, index) => <text key={time} x={x(time)} y="185" textAnchor={index === 0 ? "start" : index === 2 ? "end" : "middle"} fill="#959ca8" fontSize="10">{timeLabel(time, data.range)}</text>)}
      </svg> : <p className="py-8 text-center text-xs text-surface-muted">This metric was not reported in this period.</p>}
    </div>
  );
}
